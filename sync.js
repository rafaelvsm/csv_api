...
function sync(config, CSVModel, ItemModel) {
    const insertModel = (csv, headers) => {
        ...
        const find = { 'slug': csv.slug, 'url': csv.url };
    ...
    (function loop() {
        if (n < config.base.csv.length) {
            ...
            CSVModel.findOne({ 'slug': csv.slug, 'url': csv.url })
                .then(result => {
                    if (!result) {
                        const separator = csv.separator || ';';
                            ...
                            .on('end', () => {
                                ...
                                let insertLoop = function(index, callback){
                                    ...
                                    snappy.uncompress(batch, (err, decompressed) => {
                                        var collectionDecompressed = JSON.parse(decompressed);
                                        for (i = 0; i < collectionDecompressed.length; i++) {
                                            //if the columns value is number it converts
                                            for (var prop in collectionDecompressed[i]) {
                                                if (collectionDecompressed[i][prop] != null) {
                                                    var val = collectionDecompressed[i][prop].replace(".", "").replace(",", ".");
                                                    if (!isNaN(val)) {
                                                        var num = parseFloat(val);
                                                        if (num <= 9007199254740992) //max number							
                                                        {
                                                            collectionDecompressed[i][prop] = parseFloat(val);
                                                        }
                                                        else {
                                                            collectionDecompressed[i][prop] = val;
                                                        }
                                                    }
                                                    else if ((new Date(collectionDecompressed[i][prop]) !== "Invalid Date") &&
                                                        !isNaN(new Date(collectionDecompressed[i][prop]))) {
                                                        collectionDecompressed[i][prop] = new Date(collectionDecompressed[i][prop]);
                                                    }
                                                }
                                            }
                                        }

                                        mongoConnection.collection('items').insert(collectionDecompressed, (err, results) => {
                                            if (err) return false;
                    ...
                    {
                        console.log('Dataset already has been loaded.');
                        loop();
                    }
                });
        } else {
